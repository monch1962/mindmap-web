...
vi.mock('./AIAssistantPanel', () => ({
  default: ({ visible, onClose }: { visible: boolean; onClose: () => void }) =>
    visible ? <div data-testid="ai-assistant"><button onClick={onClose}>Close AI</button></div> : null,
}));

vi.mock('./CommentsPanel', () => ({
  default: ({ visible, onClose }: { visible: boolean; onClose: () => void }) =>
    visible ? <div data-testid="comments-panel"><button onClick={onClose}>Close Comments</button></div> : null,
}));

vi.mock('./WebhookIntegrationPanel', () => ({
  default: ({ visible, onClose }: { visible: boolean; onClose: () => void }) =>
    visible ? <div data-testid="webhook-panel"><button onClick={onClose}>Close Webhook</button></div> : null,
}));

vi.mock('./CalendarExportPanel', () => ({
  default: ({ visible, onClose }: { visible: boolean; onClose: () => void }) =>
    visible ? <div data-testid="calendar-panel"><button onClick={onClose}>Close Calendar</button></div> : null,
}));

vi.mock('./EmailIntegrationPanel', () => ({
  default: ({ visible, onClose }: { visible: boolean; onClose: () => void }) =>
    visible ? <div data-testid="email-panel"><button onClick={onClose}>Close Email</button></div> : null,
}));

vi.mock('./PresentationMode', () => ({
  default: ({ visible, onClose }: { visible: boolean; onClose: () => void }) =>
    visible ? <div data-testid="presentation-mode"><button onClick={onClose}>Close Presentation</button></div> : null,
}));

vi.mock('./ThreeDView', () => ({
  default: ({ visible, onClose }: { visible: boolean; onClose: () => void }) =>
    visible ? <div data-testid="3d-view"><button onClick={onClose}>Close 3D</button></div> : null,
}));

vi.mock('./TemplatesPanel', () => ({
  default: ({ visible, onClose }: { visible: boolean; onClose: () => void }) =>
    visible ? <div data-testid="templates-panel"><button onClick={onClose}>Close Templates</button></div> : null,
}));

vi.mock('./ThemeSettingsPanel', () => ({
  default: ({ visible, onClose }: { visible: boolean; onClose: () => void }) =>
    visible ? <div data-testid="theme-panel"><button onClick={onClose}>Close Theme</button></div> : null,
}));
...
describe('MindMapCanvas', () => {
  ...
  it('should support adding new nodes', async () => {
    ...
    await waitFor(() => {
      const nodes = screen.getAllByRole('button', { hidden: true }).filter(
        el => el.textContent?.includes('New Node')
      );
