...
function generateNewsletterEmail(tree: MindMapTree, _config: EmailConfig): string {
  ...
  lines.push('╔════════════════════════════════════════╗')
  lines.push('║       MIND MAP NEWSLETTER             ║')
  lines.push('╚════════════════════════════════════════╝')
  lines.push('')
  lines.push(`📊 ${tree.content.toUpperCase()}`)
  lines.push('')
  lines.push('─'.repeat(50))
  lines.push('')
  ...
  lines.push(`   • Total Topics: ${countNodes(tree)}`)
  lines.push(`   • Main Sections: ${tree.children?.length || 0}`)
  lines.push('')
...
export function downloadEmailHTML(
  tree: MindMapTree,
  filename: string = 'mindmap-email.html'
): void {
...
export function exportEmailTemplate(tree: MindMapTree): void {
  ...
  downloadFile(JSON.stringify(template, null, 2), 'email-template.json', {
...
export function generateWeeklyDigest(tree: MindMapTree, startDate: Date, endDate: Date): string {
  ...
  const traverse = (node: MindMapTree, depth: number = 0) => {
    ...
    if (createdDate && createdDate >= startDate && createdDate <= endDate) {
      const indent = '  '.repeat(depth)
      lines.push(`${indent}➕ NEW: ${node.content}`)
    }

    if (modifiedDate && modifiedDate >= startDate && modifiedDate <= endDate) {
      const indent = '  '.repeat(depth)
      lines.push(`${indent}✏️  UPDATED: ${node.content}`)
    }
