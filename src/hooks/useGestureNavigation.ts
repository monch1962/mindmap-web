...
export function useTouchControls(enabled: boolean = true) {
  ...
  useEffect(() => {
    ...
    if (isTouchDevice) {
      ...
      if (flowContainer) {
        flowContainer.addEventListener(
          'touchmove',
          e => {
...
export function calculateFitZoom(
  ...
  // Ensure we don't get negative values when padding is too large
  const safeAvailableWidth = Math.max(availableWidth, contentWidth * 0.1)
  const safeAvailableHeight = Math.max(availableHeight, contentHeight * 0.1)

  const scaleX = safeAvailableWidth / contentWidth
  const scaleY = safeAvailableHeight / contentHeight
